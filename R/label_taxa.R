#' @importFrom taxizedb db_download_ncbi classification
NULL

#' generates FASTA files from the ASV matrix
#'
#' @param asv_file rds file with the ASV table without chimeras
#' @param fasta_prefix name of the output file
#' @param fasta_outdir directory where the output is going to be saved
...
#' @export
condor_extract_fasta <- function(
  asv_file = "",
  fasta_prefix = "taxa",
  fasta_outdir = ".",
  condor_file = "condor_extract_fasta",
  batch_name = "dada2_extract_fasta",
  request_cores = 4,
  request_mem = "4 GB"
) {
  ...
  writeLines(
    c(
      ...
      stringr::str_c("args             = $(script_r) --asv_file $(infile)",
        "--outdir $(outdir) --prefix $(outprefix)",
        "--cores ", request_cores, sep = " "),
      ...
      stringr::str_c("script_r         = ",
        system.file("scripts/extract_fasta_before_kraken.R",
          package = "microbiome.onglab")),
      stringr::str_c("infile           = ", asv_file),
      stringr::str_c("outdir           = ", fasta_outdir),
      ...
      stringr::str_c("outprefix        = ", fasta_prefix),
...
#' parses the taxa obtained by kraken from the ids
#' @param kraken_out results obtained by kraken with `.out` suffix
#' @return a \code{tibble::tibble} with text ids parsed from the ncbi db
#' @export
parse_kraken_taxa_from_id <- function(kraken_out) {
  results <- kraken_out %>%
    dplyr::mutate(
      id_taxa = taxizedb::classification(id, db = "ncbi"),
      id_taxa = purrr::map(id_taxa, unique),
